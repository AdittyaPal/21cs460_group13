...
Linear-Perceptron Based Strategy
'''
import pandas as pd
import numpy as np
...
class LRVectorBacktester(object):
	...
	def fit_model(self, start, end, lags):
		...
		# linear regression
		regParams=np.linalg.lstsq(dataLagReturn, np.sign(self.data['returns'][lags:end]), rcond=None)[0]
		return regParams
	...
	def run_strategy(self, start_train, end_train, start_test, end_test, lags):
		...
		self.results['prediction']=np.sign(np.dot(testData, reg))
...
if __name__ == '__main__':
	...
	lrbt.run_strategy(0, 5000, 5000, 8000, lags=3)
	lrbt.plot_results()
	plt.show()
